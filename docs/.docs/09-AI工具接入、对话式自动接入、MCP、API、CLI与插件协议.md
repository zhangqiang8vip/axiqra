...
| axiqra.get_public_case | public_case_id | public_case_learning_view | case:read | 获取公开案例 |
...
| axiqra.create_candidate_seed | task_goal、coverage_gap、evidence_hint | seed_id、status | search:admin | 创建无命中候选 |
...
| search:admin | 创建 Candidate Seed |
| case:read | 读取授权 Project Case |
| case:write | 创建或编辑 Project Case |
| review:write | 审核内容 |
| audit:read | 查看审计 |
