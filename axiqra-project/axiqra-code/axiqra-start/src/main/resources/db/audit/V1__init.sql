-- Axiqra 审计库（PostgreSQL）建表脚本
-- 由 Flyway 管理，路径：classpath:db/audit/V1__init.sql
-- 此脚本一次性创建所有审计表和索引。
-- Flyway baseline：首次执行会创建 flyway_schema_history 表并记录本迁移。
-- 幂等性：所有 CREATE TABLE / CREATE INDEX 使用 IF NOT EXISTS，确保重复执行不会报错。
...
-- 仅保留 SELECT 和 INSERT，append-only 设计保证审计数据不可篡改
