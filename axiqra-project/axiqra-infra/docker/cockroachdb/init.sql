...
CREATE TABLE IF NOT EXISTS axiqra_workspace (
    ...
    workspace_type VARCHAR(20) NOT NULL DEFAULT 'personal' CHECK (workspace_type IN ('personal', 'team', 'enterprise')),
    tenant_id BIGINT NULL,
    is_deleted BOOL NOT NULL DEFAULT FALSE,
    version BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS axiqra_membership (
    ...
    PRIMARY KEY (id),
    CHECK (id > 0),
    -- Partial unique index: only active memberships (is_deleted=FALSE) must be unique per (user, workspace).
    -- This allows soft-deleted membership rows to coexist, enabling re-adding a removed user.
...
CREATE TABLE IF NOT EXISTS axiqra_engineering_trace (
    ...
    visibility_scope VARCHAR(32) NOT NULL DEFAULT 'private' CHECK (visibility_scope IN ('private', 'workspace', 'enterprise', 'public')),
...
-- ===================== Trace 证据引用 =====================
-- 首次创建由 init.sql 建表（gmt_modified NOT NULL DEFAULT now()）。
-- 已有数据库升级请执行：migrations/migration-001-add-gmt-modified-trace-evidence-ref.sql
...
CREATE TABLE IF NOT EXISTS axiqra_solution (
    ...
    visibility_scope VARCHAR(20) NOT NULL DEFAULT 'public' CHECK (visibility_scope IN ('private', 'workspace', 'enterprise', 'public')),
    license_scope VARCHAR(32) NULL,
    tenant_id BIGINT NULL,
...
-- ===================== 工具模型归因 =====================
-- 首次创建由 init.sql 建表（gmt_modified NOT NULL DEFAULT now()）。
-- 已有数据库升级请执行：migrations/migration-002-add-gmt-modified-tool-model-attribution.sql
